...
MSG91_TEMPLATE_NAME = "staff_referral_reward"
...
class ReferralRewardService:
    ...
    async def send_referral_reward(
        ...
        if not referred_person_name:
            raise HTTPException(
                status_code=400,
                detail="Referred person name is required.",
            )

        if amount is None or amount == "":
            raise HTTPException(
                status_code=400,
                detail="Reward amount is required.",
            )
        ...
        print(
            "📲 Referral Reward Notification triggered:",
            {
